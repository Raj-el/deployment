...
const mapRow = mapRowFactory(ClientsModel);
const mapRows = mapRowsFactory(ClientsModel);
...
class ClientsDAO {
  ...
  static async findById(id) {
    ...
    if (error) throw new Error(`Failed to fetch Clients record: ${error.message}`);
    return mapRow(data);
  }
  ...
  static async create(record = {}) {
    ...
    if (error) throw new Error(`Failed to create Clients record: ${error.message}`);
    return mapRow(data);
  }

  static async updateById(id, updates = {}) {
    if (!id) throw new Error('Missing identifier for Clients update');
    ...
    if (error) throw new Error(`Failed to update Clients record: ${error.message}`);
    return mapRow(data);
  }

  static async deleteById(id) {
    if (!id) throw new Error('Missing identifier for Clients delete');
    const { error } = await supabase.from(TABLE).delete().eq(PRIMARY_KEY, id);
    if (error) throw new Error(`Failed to delete Clients record: ${error.message}`);
    return true;
  }
}
